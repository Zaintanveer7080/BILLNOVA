    ...
    <div style={{ width: '210mm', minHeight: '297mm', padding: '20mm', fontFamily: 'sans-serif', color: '#000', background: '#fff' }}>
            ...
            {saleItems.map((item, index) => {
              ...
              return (
                <tr key={`${item.itemId}-${index}`} style={{ backgroundColor: index % 2 === 0 ? '#f9f9f9' : 'white' }}>
                  ...
                  <td style={{ border: '1px solid #ccc', padding: '10px', textAlign: 'right' }}>RS {safeToFixed(price)}</td>
                  <td style={{ border: '1px solid #ccc', padding: '10px', textAlign: 'right' }}>RS {safeToFixed(quantity * price)}</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <section style={{ marginTop: '20px', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <div style={{ width: '250px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0' }}><span>Subtotal</span><span>RS {safeToFixed(subTotal)}</span></div>
          {sale.discount?.value > 0 && <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0' }}><span>Discount</span><span>- RS {safeToFixed(subTotal - totalCost)}</span></div>}
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderTop: '1px solid #ccc', borderBottom: '1px solid #ccc', fontWeight: 'bold' }}><span>Total</span><span>RS {safeToFixed(totalCost)}</span></div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0' }}><span>Paid</span><span>RS {safeToFixed(paidAmount)}</span></div>
          <div style={{ display: 'flex', justifyContent: 'space-between', padding: '5px 0', fontWeight: 'bold' }}><span>Balance Due</span><span>RS {safeToFixed(balance)}</span></div>
        </div>
      </section>
