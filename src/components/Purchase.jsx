                      ...
                      <tbody>
                        {paginatedPurchases.map((purchase, index) => (
                          <motion.tr key={purchase.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className="border-b hover:bg-muted">
                            ...
                            <td className="p-2 font-semibold text-right text-blue-600">{formatMoney(purchase.totalCost, currencySymbol)}</td>
                            <td className="p-2 font-semibold text-right text-green-600">{formatMoney(purchase.paidAmount, currencySymbol)}</td>
