...
const BankTransactionLog = () => {
    ...
    const { sales, purchases, expenses, payments, banks, customers, suppliers } = data;
    ...
    const combinedTransactions = useMemo(() => {
        let allTransactions = [];
        (sales || []).forEach(s => {
            if (s.paidAmount > 0 && s.payment?.method === 'bank') allTransactions.push({
                ...
                amount: s.paidAmount, flow: 'in', bankId: s.payment.bankId, ref: s, refType: 'sale'
            });
        });
        (purchases || []).forEach(p => {
            if (p.paidAmount > 0 && p.payment?.method === 'bank') allTransactions.push({
                ...
                amount: p.paidAmount, flow: 'out', bankId: p.payment.bankId, ref: p, refType: 'purchase'
            });
        });
        (expenses || []).forEach(e => {
            if (e.paymentMethod === 'bank') allTransactions.push({
                id: `expense-${e.id}`, date: e.date, type: 'Expenses',
                party: e.category, description: e.notes,
                amount: e.amount, flow: 'out', bankId: e.bankId, ref: e, refType: 'expense'
            });
        });
        (payments || []).forEach(p => {
            if (p.method === 'bank') {
    ...
    }, [sales, purchases, expenses, payments, customers, suppliers]);
