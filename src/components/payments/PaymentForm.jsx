          ...
          {paymentMethod === 'bank' && (
            <div>
              ...
              <Select value={bankId} onValueChange={setBankId}>
                ...
                <SelectContent>
                  {(banks || []).map(bank => <SelectItem key={bank.id} value={bank.id}>{bank.name} (Balance: RS {bank.balance.toFixed(2)})</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
          )}
            ...
            <CardContent className="space-y-2">
                  ...
                  <tbody>
                    {pendingBills.length > 0 ? pendingBills.map(bill => (
                      <tr key={bill.id} className="border-b">
                        ...
                        <td className="p-2 text-right">RS {bill.due.toFixed(2)}</td>
              ...
              <div className="bg-muted p-3 rounded-lg space-y-1 text-sm">
                ...
                <div className="flex justify-between font-medium">
                  ...
                  <span className={Math.abs((parseFloat(amount) || 0) - totalAllocated) > 0.01 ? 'text-red-500' : 'text-green-500'}>RS {totalAllocated.toFixed(2)}</span>
                </div>
                <div className="flex justify-between font-medium text-blue-600">
                  <span>Unallocated:</span>
                  <span>RS {((parseFloat(amount) || 0) - totalAllocated).toFixed(2)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
