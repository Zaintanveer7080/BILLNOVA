...
import { FileDown, X, ShoppingCart, TrendingUp, TrendingDown, CreditCard, ArrowLeftRight, ArrowUpRight, ArrowDownRight, User, Building, Landmark, Hand, Hash } from 'lucide-react';
  ...
  const renderDetails = () => {
    ...
    switch (type) {
      ...
      case 'purchase':
        ...
        const profit = isSale ? getProfitOfSale(transaction) : 0;
        const totalCostNum = toNumber(transaction.totalCost);
            ...
            <Card>
                        ...
                        return (
                          <React.Fragment key={`${item.itemId}-${index}`}>
                            <TableRow>
                              ...
                              <TableCell className="text-right">{formatMoney(itemPrice, currencySymbol)}</TableCell>
                              <TableCell className="text-right">{formatMoney(itemTotal, currencySymbol)}</TableCell>
                            </TableRow>
             ...
             <div className="grid grid-cols-2 gap-8 mt-4">
                                    ...
                                    <TableBody>
                                        {relatedPayments.map(p => (
                                            <TableRow key={p.id}>
                                                ...
                                                <TableCell className="text-right">{formatMoney(p.amount, currencySymbol)}</TableCell>
                                            </TableRow>
                                        ))}
                                    </TableBody>
                                </Table>
                 ...
                 <div className="space-y-2 text-sm">
                    <div className="flex justify-between"><span>Subtotal</span><span>{formatMoney(transaction.subTotal, currencySymbol)}</span></div>
                    {transaction.discount?.value > 0 && <div className="flex justify-between"><span>Discount</span><span>- {formatMoney(toNumber(transaction.subTotal) - totalCostNum, currencySymbol)}</span></div>}
                    <div className="flex justify-between font-bold text-base border-y py-2"><span>Total</span><span>{formatMoney(totalCostNum, currencySymbol)}</span></div>
                    <div className="flex justify-between"><span>Paid</span><span>{formatMoney(paidAmount, currencySymbol)}</span></div>
                    <div className="flex justify-between font-semibold"><span>Balance Due</span><span>{formatMoney(balance, currencySymbol)}</span></div>
                    {isSale && (
                      <div className={`flex justify-between items-center font-bold pt-2 mt-2 border-t ${profit.totalProfit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {formatMoney(profit.totalProfit, currencySymbol)}
                          </span>
                      </div>
                    )}
                 </div>
            ...
            <Card>
                <CardHeader>
                    <CardTitle className="flex justify-between items-center">
                        ...
                        <span className="text-2xl font-bold text-red-600">{formatMoney(transaction.amount, currencySymbol)}</span>
                    </CardTitle>
                </CardHeader>
                <CardContent>
                 ...
                 <Card>
                    <CardHeader>
                        <CardTitle className={`flex justify-between items-center ${transaction.type === 'in' ? 'text-green-600' : 'text-red-600'}`}>
                            ...
                            <span className="text-2xl font-bold">{formatMoney(transaction.amount, currencySymbol)}</span>
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        {toNumber(transaction.discount) > 0 && <div><strong>Discount:</strong> <span className="ml-2">{formatMoney(transaction.discount, currencySymbol)}</span></div>}
