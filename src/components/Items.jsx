...
function Items() {
  ...
  const [formData, setFormData] = useState({ sku: '', name: '', category: '', purchasePrice: '', salePrice: '', openingStock: 0, unit: 'pcs', hasImei: false });
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.sku || !formData.name) {
      toast({ title: "Error", description: "SKU and Item Name are required.", variant: "destructive" });
      return;
    }

    const existingSku = items.find(item => item.sku.toLowerCase() === formData.sku.toLowerCase() && (!editingItem || item.id !== editingItem.id));
    if (existingSku) {
  ...
  const resetForm = () => {
    setFormData({ sku: '', name: '', category: '', purchasePrice: '', salePrice: '', openingStock: 0, unit: 'pcs', hasImei: false });
    setEditingItem(null);
  };
