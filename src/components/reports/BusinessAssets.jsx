        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {kpiCards.map(card => (
            <Card key={card.title}>
              ...
              <CardContent>
                <div className={`text-2xl font-bold ${card.color}`}>RS {card.value.toFixed(2)}</div>
              </CardContent>
            </Card>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center"><span className="flex items-center"><Wallet className="mr-2 h-4 w-4 text-green-500" />Cash In Hand</span> <span className="font-semibold">RS {assets.cash.toFixed(2)}</span></div>
              <div className="flex justify-between items-center"><span className="flex items-center"><Landmark className="mr-2 h-4 w-4 text-blue-500" />Bank Balances</span> <span className="font-semibold">RS {assets.bank.toFixed(2)}</span></div>
              <div className="flex justify-between items-center"><span className="flex items-center"><Package className="mr-2 h-4 w-4 text-purple-500" />Stock Value</span> <span className="font-semibold">RS {assets.stock.toFixed(2)}</span></div>
              <div className="flex justify-between items-center"><span className="flex items-center"><Users className="mr-2 h-4 w-4 text-yellow-500" />Receivables</span> <span className="font-semibold">RS {assets.receivables.toFixed(2)}</span></div>
              <div className="border-t pt-2 mt-2 flex justify-between items-center font-bold text-lg"><span >Total Assets</span> <span>RS {totalAssets.toFixed(2)}</span></div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader><CardTitle>Liabilities Breakdown</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center"><span className="flex items-center"><Building className="mr-2 h-4 w-4 text-red-500" />Payables</span> <span className="font-semibold">RS {liabilities.payables.toFixed(2)}</span></div>
              <div className="border-t pt-2 mt-2 flex justify-between items-center font-bold text-lg"><span >Total Liabilities</span> <span>RS {totalLiabilities.toFixed(2)}</span></div>
            </CardContent>
          </Card>
        </div>
      </div>
